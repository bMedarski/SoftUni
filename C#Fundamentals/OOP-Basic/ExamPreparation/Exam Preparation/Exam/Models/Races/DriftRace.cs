...
namespace Exam.Models.Races
{
    public class DriftRace : Race
    {
        ...
        public override string StartRace()
        {
            ...
            this.participants = this.participants.OrderByDescending(x =>(x.Suspension + x.Durability)).ToList();
            for (int i = 1; i <= end; i++)
            {
                if (i == 1)
                {
                ...
                st.Append($"{i}. {this.participants[i-1].Brand} {this.participants[i-1].Model} " +
                          $"{this.participants[i - 1].Suspension+this.participants[i-1].Durability}PP - ${winning}");
                if (i < end)
                {
                    st.AppendLine();
                }
